...
def get_chunks(text, max_chars=30000):
    """Split plain text into chunks at paragraph boundaries without exceeding max_chars."""
    ...
    for para in paragraphs:
        ...
        if current_length + para_len > max_chars and current_parts:
            chunks.append("\n\n".join(current_parts))
            current_parts = []
            current_length = 0
...
def translate(text, client, translation_model, temp, output_dir, n_translations, source_lang, target_lang,
    ...
    full_prompt_base = (f"You will perform a translation task from {source_lang} into {target_lang}.\n"
                        f"{prompt}\n")
    ...
    for i in range(next_num, next_num + n_translations):
        ...
        for idx, chunk_text in enumerate(chunks):
            ...
            content = (f"{full_prompt_base}\n\n"
                       f"Translate the following text in full, even if it takes time, without stopping midway or asking for clarifications. "
                       f"Return the translation ONLY. Text to translate:\n\n{chunk_text}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Translate plain text using LLMs.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("input_file", type=Path, help="Path to the plain text file to translate")
    parser.add_argument("output_dir", type=Path, help="Directory to write output into")
